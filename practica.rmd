...
```{r}
df_madrid <- airbnb_extended_clean %>% mutate(neighb_id = as.factor(cut_tree[match(Neighbourhood, cn)]))
df_madrid
```
```{r}
library(cluster)
...
aquare_meters = predict(model, data.frame(Neighbourhood = "Sol", Accommodates = 6, Bathrooms = 1, Bedrooms = 3, Beds = 3, Price = 80, Review.Scores.Rating = 80, neighb_id = "1", Square.Feet = 650, Guests.Included = 2, Extra.People = 2, Review.Scores.Rating = 80, Latitude = 40.45241, Longitude = -3.699048 ))
